...
def calc_udp_multi_process(rma_filename, file_type = 0):
    ...
    rma_file = pd.read_csv(rma_filename, index_col=0)
    df_dask = ddf.from_pandas(rma_file, npartitions=6)  # where the number of partitions is the number of cores you want to use
    #df_dask2 = df_dask.apply(calc_udp_gmm, axis=1) #.compute(scheduler='multiprocessing')
    df_dask2 = df_dask.apply(lambda x: calc_udp_gmm(x), axis=1, meta=list).compute(scheduler='multiprocessing')
    df_dask2.to_csv('data/output_udp_dask.csv')
...
if __name__ == '__main__':
    ...
    udp = calc_udp_multi_process('../Input/Lung/GSE29013_RMA.csv') #ACC.rnaseq.txt
