...
# files.download('fab_map.pdf')

# Create a KGML string for the path activity as well as KGML object.

    def process_interaction(self, group):
        row = group.iloc[0]
        inter = Bio.KEGG.KGML.KGML_pathway.Reaction()
        ...
        gr = Bio.KEGG.KGML.KGML_pathway.Graphics(parent)
        gr.name = row["intType"]
        gr.fgcolor = f"#{colors_green_to_red[fg_color]}"
        gr.bgcolor = f"#{bg_color}"
        gr.x = 10 * randrange(100)  # pr_act
        gr.y = 10 * randrange(100)  # pr_con
        gr.width = min(60, 8 * len(row["intType"]))
        gr.height = 15
        gr.type = 'circle'
        # gr.bounds = [(-29.97, -6.57), (30.03, 8.43)]

        inter.id = int(row["intID"])
        inter.name = f'inter-{row["intID"]}'
        inter.type = "irreversible"
        inter.link = 'https://www.kegg.jp/dbget-bin/www_bget?C00158>'
        inter.graphics = [gr]
        inter_en = Bio.KEGG.KGML.KGML_pathway.Entry()
        inter_en.id = int(row["intID"])
        inter_en.name = f'inter-{row["intID"]}'
        inter_en.type = "ortholog"
        inter_en.link = f'https://www.kegg.jp/dbget-bin/www_bget?hsa:4967+hsa:{row["molNum"]}'
        inter_en.graphics = [gr]
        self.pathway.add_entry(inter_en)
        self.pathway.add_reaction(inter)
        # import IPython
        # IPython.embed()
        ...
        for index, row in group.iterrows():
            # Add the reaction.
            # print(row)
            # if (int(row["intID"]) not in self.pathway.entries):
            # Add a molecule. Handle cases of empty molecule name.
            if (isinstance(row['molName'], float)):
                row['molName'] = str(row['molNum'])
            if (int(row['molNum']) not in self.pathway.entries):
                ...
                gr.fgcolor = f"#{colors_green_to_red[fg_color]}"
                gr.bgcolor = f"#{bg_color}"
                gr.x = 10 * randrange(100)  # pr
                gr.y = 10 * randrange(100)  # 0
                gr.width = min(60, 8 * len(row["molName"]))
                gr.height = 17
                gr.type = 'circle'

                en = Bio.KEGG.KGML.KGML_pathway.Entry()
                en.id = int(row["molNum"])
                en.name = f'"hsa:4967 hsa:{row["molName"]}'
                en.type = row["molType"]
                en.link = f'https://www.kegg.jp/dbget-bin/www_bget?hsa:4967+hsa:{row["molNum"]}'
                en.graphics = [gr]
                self.pathway.add_entry(en)

            # Add the molecule to the reaction.
            if (row['molRole'] == 'input' or row['molRole'] == 'agent'):
                inter.add_substrate(int(row["molNum"]))
                entry1 = int(row["molNum"])
                entry2 = int(row["intID"])
                subtypes = [("activation", "-->")]
            elif (row['molRole'] == 'output'):
                inter.add_product(int(row["molNum"]))
            ...
            # Add an edge between the molecule and the interaction.
            rel = Bio.KEGG.KGML.KGML_pathway.Relation()
            rel._setentry1(entry1)
            rel._setentry2(entry2)
            rel.type = "ECrel" #"maplink"
            rel.subtypes = [("compound", int(row["intID"]))]  # subtypes #[int(row["intID"])]
            self.pathway.add_relation(rel)
            #print("Debug")
        return
    ...
    def xmlparser(self, path_id, sample_num):
        ...
        print(f'Create KGML for path: {path_id}, sample: {sample_num}')
        ...
        pw = xml + "</pathway>"""
        self.pathway = KGML_parser.read(pw)
        paths.groupby('intID').apply(self.process_interaction)
        canvas = KGMLCanvas(self.pathway)
        # Change the colours of ortholog elements
        # Define arbitrary colours
        colorspiral = ColorSpiral()
        colorlist = colorspiral.get_colors(len(self.pathway.entries))  # orthologs
        # for key, element in self.pathway.entries.items():  # orthologs
        # for color, element in zip(colorlist, self.pathway.entries):
        #    for graphic in self.pathway.entries[element].graphics:
        #        graphic.bgcolor = rgb_to_hex(color)
        canvas.draw("testfile.pdf")
        return xml
